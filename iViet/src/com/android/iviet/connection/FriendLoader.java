...
import com.android.iviet.friend.FriendDto;
import com.android.iviet.utils.FilterLog;

abstract public class FriendLoader extends JsonLoader<List<FriendDto>> {
	public static final String TAG = "NotificaitonLoader";
	FilterLog log = new FilterLog(TAG);

	public FriendLoader(HttpUriRequest request, boolean useCache) {
	    super(request, useCache);
    }
	
	@Override
	protected List<FriendDto> handleJson(JSONArray jsonRoot) throws IOException, JSONException {
		log.d("log>>>" + "handleJson");

		List<FriendDto> mList = new ArrayList<>();
		try {
...
//				}
	...
	private List<FriendDto> parseJsonArray(JSONArray jsonArray) throws JSONException {
		...
		for (int j = 0; j < jsonArray.length(); j++) {
	    	...
	    	FriendDto FriendDto = new FriendDto(user_avatar, user_name, created_on, answer_user_name, recent_answer_date, snapshot_img, title, snapshot_content, vote_ups, number_answers, number_views, user_id, topic_icon, topic_name, theme_color, is_anonymous, question_id);
